...
public class CancelBookingHandler(CinemaDbContext db) : ICommandHandler<CancelBookingCommand,CancelBookingResult>
{
    public async Task<CancelBookingResult> Handle(CancelBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        var sale = await db.SaleScreenings
            ...
            .FirstOrDefaultAsync(ss => ss.SaleId == request.SaleId && ss.Sale.CustomerId == request.CustomerId && ss.ScreeningId == request.ScreeningId, cancellationToken);
        ...
        var saleScreeningSeats = await db.SaleScreeningSeats.FirstOrDefaultAsync(ss => ss.SaleScreeningId == sale.Id, cancellationToken);
        
        if (saleScreeningSeats is not null)
        {
            db.SaleScreeningSeats.Remove(saleScreeningSeats);
        }
