...
namespace Cinema.API.Helpers
{
    ...
    {
        public static async Task<Screening> GetScreeningAsync(CinemaDbContext db, int screeningId, CancellationToken cancellationToken)
        {
            ...
            if (screening.Date < DateTime.UtcNow)
                throw new InvalidOperationException("Screening already passed");

            return screening;
        }

        public static async Task<(bool Success, string Message)> BookScreeningAsync(CinemaDbContext db, int screeningId, IEnumerable<int> seatsId, int customerId, int saleId ,CancellationToken cancellationToken)
        {
            ...
            if (screening.IsAlreadyPassed()) throw new InvalidOperationException(
