namespace Cinema.API.Booking.Helpers
{
	public class SeatHelper
	{
		...
		public static async Task<RoomSeat> GetRoomSeatAsync(CinemaDbContext db, int seatId, CancellationToken cancellationToken = default)
		{
			var seat = await db.RoomSeats.Include(s => s.Seat).FirstOrDefaultAsync(rs => rs.Seat.Id == seatId, cancellationToken);
			if (seat == null)
		...
		/// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
		/// <returns>A task that represents the asynchronous operation. The task result contains a boolean indicating whether the seat is reserved.</returns>
		public static async Task<bool> IsSeatReservedAsync(CinemaDbContext db, int screeningId, int seatId, CancellationToken cancellationToken = default)
		{
